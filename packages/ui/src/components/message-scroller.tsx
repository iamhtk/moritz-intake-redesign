...
/**
 * Message Scroller — a styled wrapper over the headless
 * `@shadcn/react/message-scroller`, promoted from the design playground's
 * foundation Message Scroller. A chat transcript scroller built for streaming
 * chat: opening position, follow-the-live-edge auto-scroll, new-turn anchoring,
 * previous-turn peek, prepended-history preservation, visibility tracking, and
 * a jump-to-latest control.
 *
 * The behavior lives in the headless package; this wrapper only adds the
 * styling and sensible a11y defaults onto each part. The headless parts
 * already mirror their state onto the DOM, so the styling can react to
...
function MessageScrollerButton({
  ...
  /** Screen-reader label for the default icon-only content. */
  label?: string;
}) {
