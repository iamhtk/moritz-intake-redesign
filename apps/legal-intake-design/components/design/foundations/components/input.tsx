...
/**
 * Foundation Input — design-iteration shadcn Input with a layered surface.
 *
 * Mirrors the foundation Button's layered "optical" treatment: the input is
 ...
 * focus stroke too) — the input border itself does not change color on focus.
 * The wrapper is required because
 * `::before`/`::after` don't render on a bare `<input>` (a replaced element).
 * Radius is `rounded-lg` and spacing is border-compensated, matching the Button.
 *
 ...
 *
 * Kept in lockstep with `packages/ui/src/components/input.tsx` (the promoted
 * production copy): change both together or the next port will drop one
 * side's additions.
 */
  ...
  return (
    <span
      ...
      className={cn(
        ...
        // by the WRAPPER's `:hover` rather than the input's own, because the HTML spec
        // treats a control as `:hover` whenever its `<label htmlFor>` is hovered — so an
        // input-level `:hover` would misfire on label hover. The wrapper's `:hover` only
        // fires when the pointer is genuinely over the field (the spec's delegated hover
        // does not propagate to ancestors). `aria-invalid` fields keep their destructive
        // border on hover, matching the reference.
