...
/**
 * Foundation Select — design-iteration shadcn Select with a layered surface.
 *
 * The trigger mirrors the foundation Input: it's wrapped in a
 ...
 * border) and an `after` layer drawing the inset 2px primary focus ring — while
 * the border + border-compensated mobile-first padding live on the inner Radix
 * Trigger button. Height is padding-driven (no fixed `h-*`), so the trigger
 * tracks the foundation Input at both breakpoints. The hover border is driven by
 * the WRAPPER's `:hover` (not the button's) so a label-forwarded `:hover` doesn't
 * misfire — see input.tsx for the rationale.
 *
 * The content panel and items reuse the foundation Dropdown Menu's language: a
 * translucent blurred surface carried by a ring, `rounded-[0.5rem]` items with
 * mobile-first padding, muted icons, and a saturated `primary` focus highlight.
 * Colors come from theme tokens, not `dark:` utilities (this app has no
 * class-based dark variant).
 *
 * TODO(@repo/ui): fold this back into `packages/ui/src/components/select.tsx`
  ...
  return (
    <span
      ...
      className={cn(
        ...
        // 1px smaller). Use the `shadow` token (not `shadow-sm`) to match the
        // reference: its Tailwind-v4 `shadow-sm` equals this repo's `shadow` token,
        // whereas our `shadow-sm` is the far fainter 0.05 variant. Matches the
        // foundation Input.
        ...
        // `:hover` on the trigger button doesn't misfire (see input.tsx).
    ...
    >
      <SelectPrimitive.Trigger
        ...
        className={cn(
          ...
          // Mobile-first padding drives height (no fixed `h-*`). `sm` is the compact
          // scale (mirrors the shared Select's shorter trigger); `default` tracks the
          // foundation Input height.
          size === 'sm'
