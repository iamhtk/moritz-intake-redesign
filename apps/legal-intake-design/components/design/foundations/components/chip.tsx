...
/**
 * Foundation Chip — a spacious, pill-shaped trigger that pairs a leading icon
 * with a label. Used for composer quick actions (e.g. "Create an image", "Write
 ...
 * primary CTAs. Distinct from the intake `ChipSelect` composite: this is a
 * stateless trigger — no `selected`/toggle state.
 *
 * Shares the foundation Button/Toggle design language rather than the Badge's:
 * outline-based focus (`focus-visible:outline-*`), `border-border` on the
 * outline variant, the mobile-first border-compensated size scale (a larger
 * touch target on small screens collapsing to compact desktop density at
 ...
 * `data-slot`) to match the rest of the library. Renders a
 * `<button type="button">` by default, or any element via `asChild` (e.g. an
 * `<a>` for link chips).
 *
...
const chipVariants = cva(
  ...
  // still show the `not-allowed` cursor on hover (mirroring the foundation
  // Button); hover styling is gated behind `not-disabled:`. Icon sizing and the
  // outline-based focus match the foundation Button/Toggle.
  "inline-flex w-fit shrink-0 cursor-pointer items-center justify-center gap-2 rounded-full border border-transparent whitespace-nowrap font-medium transition-all outline-none focus-visible:outline-solid focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring disabled:cursor-not-allowed disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg[data-icon=inline-start]]:-ms-0.5 [&_svg[data-icon=inline-end]]:-me-0.5",
  {
    variants: {
      ...
      size: {
        ...
        // step roomier than the foundation Button so chips still read spacious.
        // As on the Button, `default` and `lg` share the type scale and differ
        // by padding only; `sm` steps the text down.
        default:
