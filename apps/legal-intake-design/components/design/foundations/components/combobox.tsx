 ...
 * `packages/ui/src/components/combobox.tsx` — same exports and props (Base UI),
 * so no call sites change. To lift it: copy this file over and repoint the
 * `InputGroup`/`InputGroupAddon`/`InputGroupInput` import from
 * `@/components/design/foundations/components/input-group` to
 * `@repo/ui/components/input-group` (functionally identical). The other imports
 * (`@base-ui/react`, `@repo/ui/icons`, `@repo/ui/lib/utils`) already resolve.
 * Additive/changed vs the current @repo/ui combobox: it adds `ComboboxItemLabel`
 * and `ComboboxItemDescription` exports, uses a left-aligned selected check
 * (matching the Select), and drops the `Button` dependency (trigger/clear are
 * plain flush buttons). Every token used already exists in @repo/ui's
 * globals.css.
 */
