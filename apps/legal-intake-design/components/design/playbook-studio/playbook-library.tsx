...
export function PlaybookLibrary({ basePath }: PlaybookLibraryProps) {
  ...
  const { playbooks: allPlaybooks, refresh } = usePlaybooks();
  const playbooks = useMemo(
    () => allPlaybooks.filter((p) => !HIDDEN_PLAYBOOK_IDS.has(p.id)),
    [allPlaybooks],
  );
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <Muted>
            ...
            negotiation standards - surgical edits, not wholesale rewrites.
          </Muted>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Badge variant="secondary" className="px-2 py-0.5 text-[0.8125rem]">
          <Layers aria-hidden />
